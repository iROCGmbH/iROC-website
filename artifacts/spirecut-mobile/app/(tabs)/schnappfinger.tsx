...
export default function SchnappfingerScreen() {
  ...
  const tf = t.tf;
  const symptoms = tf.symptoms as unknown as string[];
  const classicItems = tf.classicItems as unknown as string[];
  const modernItems = tf.modernItems as unknown as [string, string][];
    ...
    >
      ...
      <View style={[styles.hero, { backgroundColor: colors.primary, paddingTop: topPad + 16 }]}>
        ...
        <View style={[styles.badge, { backgroundColor: 'rgba(255,255,255,0.2)' }]}>
          <Text style={[styles.badgeText, { color: colors.primaryForeground }]}>{tf.badge}</Text>
        </View>
        <Text style={[styles.heroTitle, { color: colors.primaryForeground }]}>{tf.heroTitle}</Text>
        <Text style={[styles.heroDesc, { color: 'rgba(255,255,255,0.85)' }]}>{tf.heroDesc}</Text>
        <Pressable
        ...
        >
          ...
          <Text style={[styles.heroBtnText, { color: colors.primary }]}>{tf.findNearby}</Text>
        </Pressable>
      </View>

      {/* What is it */}
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.cardTitle, { color: colors.foreground }]}>{tf.whatTitle}</Text>
        <Text style={[styles.cardBody, { color: colors.mutedForeground }]}>{tf.whatDesc}</Text>
      </View>

      {/* Symptoms */}
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.cardTitle, { color: colors.foreground }]}>{tf.symptomsTitle}</Text>
        {symptoms.map((s, i) => (
      ...
      <View style={styles.sectionHeader}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>{tf.treatmentTitle}</Text>
      </View>
      ...
      <View style={[styles.compCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={styles.compHeader}>
          ...
          <Text style={[styles.compTitle, { color: colors.mutedForeground }]}>{tf.classicTitle}</Text>
        </View>
      ...
      <View style={[styles.compCard, styles.modernCard, { backgroundColor: colors.primary }]}>
        <View style={styles.compHeader}>
          ...
          <Text style={[styles.compTitle, { color: colors.primaryForeground }]}>{tf.modernTitle}</Text>
        </View>
      ...
      <View style={[styles.ctaSection, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.ctaTitle, { color: colors.foreground }]}>{tf.ctaTitle}</Text>
        <Text style={[styles.ctaDesc, { color: colors.mutedForeground }]}>{tf.ctaDesc}</Text>
        <View style={styles.ctaButtons}>
          ...
          >
            ...
            <Text style={[styles.ctaBtnText, { color: colors.primaryForeground }]}>{tf.findNearby}</Text>
          </Pressable>
          <Pressable
            style={[styles.ctaBtn, styles.ctaBtnOutline, { borderColor: colors.border }]}
            onPress={() => {
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              router.push('/how-it-works' as never);
            }}
          >
            <Feather name="info" size={14} color={colors.foreground} />
            <Text style={[styles.ctaBtnText, { color: colors.foreground }]}>{tf.watchVideo}</Text>
          </Pressable>
        </View>
      </View>
    </ScrollView>
