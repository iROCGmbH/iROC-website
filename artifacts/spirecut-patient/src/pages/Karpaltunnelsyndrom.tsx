...
export default function Karpaltunnelsyndrom() {
  const { t } = useTranslation();
  const heroUrl = useMedia("hero-ct", `${BASE}kts-hero-a.jpg`);
    ...
    <div className="flex flex-col w-full bg-white">
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-gray-600 mb-8 leading-relaxed">{t("ct.heroDesc")}</p>
              <Link href="/arzt-finden">
                <span className="inline-flex items-center gap-2 h-12 px-7 text-sm font-semibold bg-primary text-white hover:bg-primary/90 transition-colors rounded cursor-pointer">
                  {t("ct.findDoctor")} <ArrowRight className="h-4 w-4" />
                </span>
              </Link>
            </div>
            {heroUrl && (
      ...
      <section className="py-20 lg:py-24">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-3xl font-bold text-gray-900 mb-4 text-center">{t("ct.whatTitle")}</h2>
            <div className="w-10 h-0.5 bg-primary mx-auto mb-10" />
            <p className="text-lg text-gray-600 mb-8 leading-relaxed text-center">{t("ct.whatDesc")}</p>

            <div className="bg-gray-50 border border-gray-100 p-8 lg:p-10 rounded-xl mt-10">
              <h3 className="text-lg font-bold text-gray-900 mb-5 flex items-center">
                <Activity className="mr-2.5 text-primary h-5 w-5" /> {t("ct.symptomsTitle")}
              </h3>
              <ul className="grid sm:grid-cols-2 gap-3.5">
                {(t("ct.symptoms", { returnObjects: true }) as string[]).map((symptom, idx) => (
      ...
      <section className="py-20 lg:py-24 bg-gray-900 text-white">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl mx-auto text-center mb-14">
            <h2 className="text-3xl font-bold text-white mb-4">{t("ct.treatmentTitle")}</h2>
            <p className="text-gray-400">{t("ct.treatmentSubtitle")}</p>
          </div>

          <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            <div className="bg-white/5 p-8 rounded-xl border border-white/10">
              <h3 className="text-base font-bold text-gray-300 mb-5 uppercase tracking-wider">{t("ct.classicTitle")}</h3>
              <ul className="space-y-3.5 text-gray-400 text-sm">
                {(t("ct.classicItems", { returnObjects: true }) as string[]).map((item, i) => (
            ...
            <div className="bg-white p-8 rounded-xl border-2 border-primary shadow-xl relative">
              ...
              <h3 className="text-base font-bold text-gray-900 mb-5 uppercase tracking-wider">{t("ct.modernTitle")}</h3>
              <ul className="space-y-3.5 text-sm">
                {(t("ct.modernItems", { returnObjects: true }) as [string, string][]).map(([label, desc], i) => (
                  <li key={i} className="flex items-start gap-3">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 lg:px-8 text-center max-w-3xl">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">{t("ct.ctaTitle")}</h2>
          <p className="text-gray-500 mb-10">{t("ct.ctaDesc")}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/arzt-finden">
              <span className="inline-flex items-center justify-center gap-2 h-13 px-8 py-3.5 text-base font-semibold text-white bg-primary hover:bg-primary/90 transition-colors rounded cursor-pointer">
                {t("ct.findNearby")} <ArrowRight className="h-5 w-5" />
              </span>
            </Link>
