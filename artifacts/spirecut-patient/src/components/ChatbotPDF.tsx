 ...
 * { text, bold } segments.
 */
export function parseInline(text: string): Array<{ text: string; bold: boolean }> {
...
export function stripFollowUpMetadata(content: string): string {
  let remaining = content;
  let clean = '';

  while (true) {
    const markerStart = remaining.search(FOLLOWUP_MARKER_START_RE);
    if (markerStart === -1) return `${clean}${remaining}`.trimEnd();
    clean += remaining.slice(0, markerStart);
    const markerEnd = remaining.indexOf('-->', markerStart);
    if (markerEnd === -1) return clean.trimEnd();
    remaining = remaining.slice(markerEnd + 3);
  }
}
...
export function parseMarkdown(content: string): MdBlock[] {
  const lines = stripFollowUpMetadata(content).split('\n');
...
export function ConversationDocument({ messages, lang, timestamp, assetBase }: ConversationDocProps) {
  ...
  return (
    ...
    >
      <Page size="A4" style={s.page}>
        ...
        {/* Messages */}
        {messages.map((msg) => (
          <View key={msg.id} style={s.messageWrap}>
            {msg.role === 'user' ? (
              <>
