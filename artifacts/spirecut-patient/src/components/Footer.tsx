    ...
    <footer className="bg-white border-t border-gray-200 pt-14 pb-8">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-12">
          ...
          {/* Pages */}
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">{t("footer.pages")}</h4>
            <ul className="space-y-2.5">
              {PAGE_LINKS.map(({ href, footerLabelKey }) => (
                <li key={href}>
                  <Link href={href}>
                    <span className="text-sm text-gray-600 hover:text-primary transition-colors cursor-pointer">{t(footerLabelKey)}</span>
                  </Link>
                </li>
              ))}
            </ul>
          ...
          {/* Legal + Contact */}
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">{t("footer.legal")}</h4>
            <ul className="space-y-2.5 mb-8">
              {LEGAL.map((p) => (
                <li key={p.href}>
                  <Link href={p.href}>
                    <span className="text-sm text-gray-600 hover:text-primary transition-colors cursor-pointer">{t(p.labelKey)}</span>
                  </Link>
                </li>
              ))}
            </ul>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">{t("footer.sales")}</h4>
